...
class CalculoSBC(models.TransientModel):
    ...
    def print_sbc_report(self):
        ...
        dias_mes_uno = (self.primer_fecha_fin - self.primer_fecha_inicio).days
        dias_mes_dos = (self.segundo_fecha_fin - self.segundo_fecha_inicio).days
    ...
    def change_sbc(self):
        ...
        for employee, payslips in employees.items():
            ...
            if not employee.contract_id:
                continue
            contrato = employee.contract_id[0]
            date1 = datetime(day=datetime.strptime(self.segundo_fecha_fin, "%Y-%m-%d").day, month=datetime.strptime(self.segundo_fecha_fin, "%Y-%m-%d").month, year=datetime.strptime(self.segundo_fecha_fin, "%Y-%m-%d").year) + timedelta(days=1)
            date2 = datetime(day=datetime.strptime(contrato.date_start, "%Y-%m-%d").day, month=datetime.strptime(contrato.date_start, "%Y-%m-%d").month, year=datetime.strptime(contrato.date_start, "%Y-%m-%d").year)
            no_dias = date1- date2
            no_anos = no_dias.days / days_year
            dias_anos = math.ceil(no_anos)
            if dias_anos < 1.0: 
